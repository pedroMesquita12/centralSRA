...
export default function Certificados() {
    ...
    async function carregarArquivos() {
      try {
        ...
        const response = await fetch(
          `${API_URL}/certificados/aluno/${user.id}`
        );
        ...
        if (!response.ok) {
          throw new Error(
            data.message || "Erro ao carregar arquivos"
          );
        }

        const arquivosFormatados: ArquivoAluno[] =
          data.map((item: any) => ({
            ...
            quantidadeHoras: Number(
              item.quantidade_horas || 0
            ),

            dataEmissao: item.data_emissao,

            arquivoNome:
              item.nome_arquivo || "Arquivo",

            arquivoUrl:
              item.url_publica || item.url_arquivo,

            status:
              item.status_certificado || "PENDENTE",
          }));
  ...
  const certificados = arquivos.filter(
    (item) => item.tipo === "certificado"
  );

  const relatorios = arquivos.filter(
    (item) => item.tipo === "relatorio"
  );
  ...
  const totalHoras = useMemo(() => {
    return arquivos
      ...
      .reduce(
        (acc, item) =>
          acc + Number(item.quantidadeHoras || 0),
        0
      );
  }, [arquivos]);
  ...
  const validarArquivo = () => {
    ...
    const permitido = extensoesPermitidas.some(
    ...
    if (
      ...
      alert(
        "Relatórios devem ter entre 1 e 2 horas."
      );
    ...
    if (lista.length === 0) {
      return (
        <div
          ...
          "
        >
          <FileText className="size-10 text-white/50 mx-auto mb-3" />
      ...
      >
        <CardHeader>
          <CardTitle className="text-white flex items-center gap-2">
            <FileBadge2 className="size-5 text-[#B8AFFF]" />
            Arquivos Acadêmicos
          </CardTitle>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <button
                onClick={() =>
                  setFiltro("certificado")
                }
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                  filtro === "certificado"
                    ? "bg-[#7C5DFA]/18 text-[#F5F3FF] border border-[#B8AFFF]/10"
